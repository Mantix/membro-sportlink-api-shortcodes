...
class MembroSportlinkClient {
  ...
  public function __construct($apiKey, $apiPath, $clubName = null) {
    ...
    curl_setopt($ch, CURLOPT_URL, MembroSportlinkClient::BASEURI . "$init_path");
  ...
  public function doRequest($url_path = '/teams', $extra = NULL, $get_status = false) {
    ...
    curl_setopt(
      $ch,
      CURLOPT_URL,
      MembroSportlinkClient::BASEURI . $url_path . '?PHPSESSID=' . $this->session_id . '&hash=' . $hash . '&' . $extra
    );
    curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
    ...
    if (isset($json_data) && property_exists($json_data, 'List')) {
      if (
        property_exists($json_data->List[0], 'Datum') &&
        ...
        is_array($json_data->List) && count($json_data->List) > 1
  ...
  public function getData($url_path = '/teams', $extra = NULL, $template_file = NULL, $fields = NULL) {
    ...
    if (
      ...
      $extra = str_replace(
        ...
        array(
          'weeknummer=' . $wn_current,
          'weeknummer=' . $wn_previous,
          'weeknummer=' . $wn_next
        ),
        $extra
      );
      if ($parameters['weeknummer'] === 'P') {
        $parameters['weeknummer'] = $wn_previous;
      } elseif ($parameters['weeknummer'] === 'C') {
        $parameters['weeknummer'] = $wn_current;
      } elseif ($parameters['weeknummer'] === 'N') {
        $parameters['weeknummer'] = $wn_next;
      }
    }
    ...
    RainTPL::configure('cache_dir', ABSPATH . 'wp-content/uploads/shortcode-membro-sportlink-api/cache/');
    ...
    $cachetime = esc_attr(get_option('membro_sportlink_api_cachetime')) * 60;
